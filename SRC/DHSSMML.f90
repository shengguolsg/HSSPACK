!  
   SUBROUTINE DHSSMML( A,LDA,M,N,B,LDB,K,tol,prank,ni,ncol,time,info)
      use aux_hss
      use BasicMM
      use ConstructHssd
!
...
! =======
!
! This routine assume A is an HSS matrix and computes the multiplication of B and A, 
...
!   **************** HSS Matrix-Matrix Multiplication *******************
    deallocate( work )
    Allocate( WW(LDA,2*ncol), SX(lvl*n*lda) )

    nc = floor( dble(K) /ncol )
    call cpu_time( time2 )
    DO i = 1, nc
       call fastHssmmL( DD,H,PH,blk,B((i-1)*ncol+1:i*ncol,1:N),tr,ltr,ncol,n,TRE,lvl,WW,SX )
    End Do
    nlast = K - nc * ncol
    IF( nlast .ne. 0 ) THEN
       i = nc+1
       call fastHssmmL( DD,H,PH,blk,B((i-1)*ncol+1:N,1:N),tr,ltr,nlast,n,TRE,lvl,WW,SX )
    END IF
    call cpu_time( time1 )        
    time(2) = time2 - time1
