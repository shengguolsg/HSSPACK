      SUBROUTINE MDLASD1(NL, NR, SQRE, D, ALPHA, BETA, U, LDU, VT, LDVT, &
...
!     from the overall problem. When the size of secular equation is larger 
!     than 400, the HSS matrix techniques would be used. 
!
!  Arguments
!  =========
!
...
!     .. Local Scalars ..
      ...
      INTEGER            COLTYP, I, IDX, IDXC, IDXP, IQ, ISIGMA, IU2, &
                         ...
                         IDIFL, IDIFR, IAPHAU, IAPHAV, ILEFT, J, IZZ, IUF
      DOUBLE PRECISION   ORGNRM
!     ..
!     .. External Subroutines ..
      EXTERNAL           DLAMRG,DLASCL, MDLASD2, DLASD3, XERBLA,MDLASD8, &
                         DGER
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
...
!
      CALL MDLASD2( NL, NR, SQRE, K, D, WORK( IZ ), ALPHA, BETA, U, LDU, &
                   VT, LDVT, WORK( ISIGMA ), WORK( IU2 ), LDU2, &
                   WORK( IVT2 ), LDVT2, IWORK( IDXP ), IWORK( IDX ), &
                   IWORK( IDXC ), IDXQ, IWORK( COLTYP ), HFLAG, INFO )
!
...
!     HFLAG = TRUE: use LAPACK routine; HFLAG = FALSE: use HSS matrices
      ...
      ELSE
!
!     After calling dlasd2, the first K columns (rows) of U and VT store
...
!
         WORK( IZZ:IU2-1 ) = WORK( IZ: (IZ+K-1) )  ! ZZ = Z
!
!        Store the first K cols of U. 
         CALL DLACPY( 'A', LDU2, K, U, LDU, WORK(IU2), LDU2 )
         CALL DLACPY( 'A', K, LDVT, VT, LDVT, WORK(IVT2), K)  ! Store the first K rows of VT
         CALL DLACPY( 'A', LDU2, 1, U, LDU, WORK(IUF), LDU2)  ! First col of U
!
!!$!        Check for multiple roots
!!$         DO I = ISIGMA, ISIGMA+K-1
!!$            IF( WORK(I) .EQ. WORK(I+1) ) THEN
!!$               WRITE(*,*) 'There is multiple singular values'
!!$            END IF
!!$         END DO
!!$!        Check for zero entry 
!!$         DO I = 1, K
!!$            IF( WORK(I) .EQ. ZERO ) THEN
!!$               WRITE(*,*) 'There are zeros in Z'
!!$            END IF
!!$         END DO
         CALL MDLASD8( K,D,WORK(IZ),WORK(ISIGMA),WORK(IDIFL), &
             WORK(IDIFR), WORK(IAPHAU), WORK(IAPHAV),WORK(IZZ), &
             WORK(IU2),LDU2,WORK(IVT2),LDVT2,WORK(ILEFT),INFO)
!
!        Update U and VT
         CALL DLACPY( 'A', K, LDVT2, WORK(IVT2), K, VT, LDVT ) ! Update the first K rows of VT
!
...
!        IUF is a column vector and IAHPAU is a row vector. 
         CALL DGER( LDU2,K,1.0D0,WORK(IUF),1,WORK(IAPHAU),1,WORK(IU2), &
                    LDU2 )
         CALL DLACPY( 'A',LDU2,K,WORK(IU2),LDU2,U,LDU )
!
      END IF ! ( HFLAG )
