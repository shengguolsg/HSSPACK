  SUBROUTINE MDLASD2( NL, NR, SQRE, K, D, Z, ALPHA, BETA, U, LDU, VT, &
                     LDVT, DSIGMA, U2, LDU2, VT2, LDVT2, IDXP, IDX, &
                     IDXC, IDXQ, COLTYP, HFLAG, INFO )
!
...
!
      DO 60 I = 2, N
         DSIGMA( I ) = D( IDXQ( I ) )
         U2( I, 1 ) = Z( IDXQ( I ) )
         IDXC( I ) = COLTYP( IDXQ( I ) )  ! labels of columns of U
   60 CONTINUE
!
...
!
      DO 70 I = 2, N
         IDXI = 1 + IDX( I )
         D( I ) = DSIGMA( IDXI )    ! Now D is in right order
         Z( I ) = U2( IDXI, 1 )
...
!
         ...
         ELSE   ! not 
            ...
            U2( K, 1 ) = Z( JPREV )   ! store current Z(J)
...
!
         PSM( 1 ) = 2  ! start from the second column, first column is e_NLP1
         PSM( 2 ) = 2 + CTOT( 1 )        ! starting col for type 2
...
!
         DO 150 J = 2, N
            ...
            PSM( CT ) = PSM( CT ) + 1 ! Each type increase by one
 150     CONTINUE
!
...
!
  ...
  180    CONTINUE
      ...
      END IF
      IF( M.GT.N ) THEN
         CALL DCOPY( M, VT( M, 1 ), LDVT, VT2( M, 1 ), LDVT2 ) ! last row of VT2
      END IF
!
      IF( .NOT. HFLAG ) THEN
!
...
!
         ...
         CALL DLACPY( 'A', N, N, U2( 1, 1 ), LDU2, U( 1, 1 ), &
                      LDU )
         CALL DLACPY( 'A', N, M, VT2( 1, 1 ), LDVT2, VT( 1, 1 ), &
                      LDVT )
!
      ELSE  IF( N.GT.K ) THEN
!
